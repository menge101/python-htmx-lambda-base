...
from lib.components import session
...
@fixture
def session_data_none() -> session.SessionData:
    return cast(session.SessionData, {})


@fixture
def session_data(base_time, session_id):
    return cast(session.SessionData, {"id_": session_id, "ttl": base_time.timestamp()})


def test_build_no_session(connection_thread_mock, mocker, session_data_none):
    mocker.patch("lib.components.session.cookie.expiration_time", return_value=0)
    mocker.patch("lib.components.session.cookie.expiration_as_ttl", return_value="0")
    mocker.patch("lib.components.session.uuid1", return_value="1")
...
def test_build_with_session(base_time, connection_thread_mock, mocker, session_id, session_data):
    mocker.patch("lib.components.session.cookie.expiration_as_ttl", return_value=base_time.timestamp())
...
def test_act_no_session(connection_thread_mock, mocker, resource_mock, session_data_none):
    mocker.patch("lib.components.session.cookie.expiration_time", return_value=datetime.fromtimestamp(0))
    mocker.patch("lib.components.session.uuid1", return_value="1")
...
def test_update_session_raises_client_error(connection_thread_mock, mocker, resource_mock, session_data):
    ...
    mocker.patch("lib.components.session.botocore.exceptions.ClientError", MockException)

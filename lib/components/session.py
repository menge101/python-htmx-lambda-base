...
from lib.core import cookie, return_, threading
from typing import cast, Literal, NewType, Optional
...
@xray_recorder.capture("## Creating session")
def act(
...
def build(
    ...
    **_kwargs,
    ...
    session_cookie = cookie.Cookie(
        ...
        http_only=True,
        expires=exp,
        same_site=cast(Literal["Lax", "None", "Strict"], "Strict"),
    )
...
def get_session_data(session_id: str, table_connection_thread: threading.ReturningThread) -> SessionData:
    _, _, tbl = cast(threading.ConnectionThreadResultType, table_connection_thread.join())
...
def update_session(connection_thread: threading.ReturningThread, session_data: dict[str, Optional[str]]) -> None:
    ...
    _, _, ddb_tbl = cast(threading.ConnectionThreadResultType, connection_thread.join())
    try:
